...
- Agent instance version `tool_config`, `mcp_config`, `workflow_config`, and `policy_config` are validated and applied during run execution, including model-loop shortlisting, tool aliases, optional interleaved multi-tool execution, and per-turn tool-call limits.
