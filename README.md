...
```bash
DURACLAW_ARTIFACT_PROCESSOR_PROVIDER=together
DURACLAW_ARTIFACT_PROCESSOR_API_KEY=...
DURACLAW_ARTIFACT_PROCESSOR_MODEL=moonshotai/Kimi-K2.5
DURACLAW_ARTIFACT_PROCESSOR_MODALITIES=image,document
```
...
- Agent instance version `tool_config`, `mcp_config`, `workflow_config`, and `policy_config` are validated and applied during run execution, including domain-neutral model-loop shortlisting, per-agent tool metadata hints, tool aliases, optional interleaved multi-tool execution, and per-turn tool-call limits.
