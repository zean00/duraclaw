...
func (CreateReminderTool) Parameters() map[string]any {
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"repeat_until":            map[string]any{"type": "string", "description": "Optional RFC3339 stop boundary for bounded recurring reminders. Required when the user expresses a duration such as 'for 3 days' or '3 hari kedepan'. The reminder will not be scheduled again after this time."},
			"repeat_count":            map[string]any{"type": "integer", "description": "Optional maximum number of firings for count-bounded reminders such as '9 times'. For duration-bounded reminders, prefer repeat_until; 0 means no count limit."},
