...
func (s *Service) consolidateProfileBestEffort(ctx context.Context, now time.Time, session db.MonitoredSession) {
	...
	resp, err := s.providers.ChatWithFallback(ctx, s.modelConfig, []providers.Message{
		{Role: "system", Content: "You consolidate a user's stored profile memory. Merge only clear duplicates or near-duplicates. Preserve separate facts/preferences that are conditional, complementary, or potentially different. If a newer explicit user preference or fact replaces an older conflicting one, keep only the current active item in clean canonical wording and mark the obsolete item as a duplicate/delete. Do not include lifecycle words such as supersedes, replaced, obsolete, previous, or old in content. Return JSON only: {\"memory_updates\":[{\"id\":\"existing-id\",\"type\":\"fact\",\"content\":\"canonical active text\",\"duplicate_ids\":[\"duplicate-id\"]}],\"preference_updates\":[{\"id\":\"existing-id\",\"category\":\"style\",\"content\":\"canonical active text\",\"condition\":{},\"duplicate_ids\":[\"duplicate-id\"]}],\"memory_delete_ids\":[\"duplicate-id\"],\"preference_delete_ids\":[\"duplicate-id\"]}. Use only existing ids. Put duplicate ids in duplicate_ids on their canonical item and/or the top-level delete arrays. Do not delete an item unless it is represented by another canonical item."},
