...
func (s *Store) MarkRunMessagesHiddenFromHistory(ctx context.Context, runID, role string, metadata any) error {
	metadataJSON, _ := json.Marshal(metadata)
	if string(metadataJSON) == "null" {
		metadataJSON = []byte(`{}`)
	}
	_, err := s.pool.Exec(ctx, `
		...
		SET content = jsonb_set(
			...
			'{metadata}',
			COALESCE(content->'metadata', '{}'::jsonb) || $2::jsonb || '{"context_excluded":true,"visible_in_history":false,"visibility":"hidden"}'::jsonb,
			true
		)
		WHERE run_id=$1 AND ($3='' OR role=$3)`, runID, metadataJSON, strings.TrimSpace(role))
	return err
