...
For WhatsApp Web group sessions, Nexus should store passive group messages locally and only call Duraclaw when the bot is natively mentioned. Mentioned turns can include `structured_data` with `data.kind: "whatsapp_group_context"` so Duraclaw can use a capped recent group transcript without forwarding every passive group message into ACP history.
...
Before the normal assistant model, tools, workflows, recommendations, delegation, or MCP calls run, Duraclaw may call a combined scope, intent, action, and moderation judge using the snapshotted profile configuration. Local moderation word/pattern rules can block before that LLM call. The judge can also emit `requires_action` and `action_intents`; enabled tool selection treats `requires_action:no` as a no-tool turn and uses matching action intents to boost tools with corresponding metadata labels. If moderation denies a request, the raw user message is persisted for audit with `visible_in_history:false` and `context_excluded`, while the visible warning response is also `context_excluded`; both are skipped by prompt and session-summary context builders. If the judge determines the request is out of scope, the run completes with the configured out-of-scope response and no side-effecting execution occurs. The denied user message and refusal response are persisted for audit/display with `context_excluded`, so they are not used as future main-agent model context. Scope judging can still read scope-denied turns during implicit follow-up checks to prevent a follow-up from bypassing the original denial.
...
- Generated from an external promotion/offer/feature payload through a specific agent profile and immediately fanned out.
...
Implementation status: broadcast creation accepts explicit targets and database-resolved target selections for all customer users, specific user IDs, users attached to an agent instance, reminder subscribers, and a constrained segment selector over durable session fields (`user_id_prefix`, `session_id_prefix`, `agent_instance_id`, and RFC3339 `updated_since`). Arbitrary SQL segment queries are intentionally not accepted. Direct broadcasts create per-target outbound intents immediately. Generated broadcasts accept `generation.mode` of `agent_per_instance` or `per_user`; Duraclaw creates trusted system runs through `generation.agent_instance_id`, suppresses direct run outbound, and the scheduler converts completed generated text into per-target broadcast outbound intents. Broadcasts can carry an optional caller `external_broadcast_id`, emit `broadcast_reference` artifacts, and suppress targets whose session channel is blocked by session recommendation policy. Nexus delivery status callbacks accept `sent_to_nexus`, `delivered`, `failed`, and `cancelled`, update outbound and broadcast target state transactionally, and persist durable outbound acknowledgement observability events. Duraclaw can push outbound intents to Nexus one at a time or in topic-grouped bulk batches when `NEXUS_OUTBOUND_BULK_URL` is configured.
...
Implementation status: model, tool, MCP, and artifact processor call starts/completions write both run events and durable observability events. In-process metrics expose counters plus duration count/sum/bucket series through `/metrics`, including run queue lag, run duration, model token usage, async write drop/degrade/flush counts, and model/tool/MCP/artifact processor/workflow-node durations. OpenTelemetry SDK tracing/metrics can export to OTLP HTTP collectors while `/metrics` remains available for local inspection. Non-critical observability sidecars and high-volume run events can flow through `async_write_jobs` with degradation and drop accounting. Buffered run events currently cover streaming model deltas, agent activity status events, scope-judge audit events, and prompt-injection block telemetry; critical state transitions and external side-effect records remain synchronous. Inbound `traceparent` and `X-Trace-ID` headers are parsed into durable trace metadata, checkpoints include trace metadata, MCP and artifact processor execution context can propagate trace headers, and policy audit payloads redact common secret and personal-data patterns.
